$(document).on('turbolinks:load', ()=> {
  const buildFileField = (index)=> {
    const html = `
      <div data-index="${index}" class="js-file_group">
        <input class="js-file" type="file" name="post[images_attributes][${index}][picture]" id="post_images_attributes_${index}_picture">
      </div>
    `;
    return html;
  };
  ...
  $('#image-box').on('click', '.delete-box', function() {
    ...
    $(`#post_images_attributes_${targetIndex}_picture`).remove();
